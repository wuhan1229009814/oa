$(function () {
  ...
  QUnit.test('should throw an error when templates contains multiple top-level elements', function (assert) {
    assert.expect(1)
    assert.throws(function () {
      $('<span data-toggle="popover" data-title="some title" data-content="some content">some text</span>')
        ...
        .bootstrapPopover('show')
    }, new Error('popover `templates` option must consist of exactly 1 top-level element!'))
  })
